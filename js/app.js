$(document).ready(function () {
  ...
  $(document).on("click", ".js-add-fav", function (e) {
    ...
    if (!key) {
      console.warn("Add-to-favourites button clicked but missing data-key", {
        dataset: $btn.data(),
        element: $btn,
      });
      showAlert(
        "Could not add to favourites: missing book key (check console).",
        "failed"
      );
      return;
    }
  ...
  (async function () {
    ...
    try {
      const broad = await fetchJson(
        "https://openlibrary.org/search.json?q=the&limit=200"
      );
      const docs =
        broad && broad.data && broad.data.docs ? broad.data.docs : [];
      const counts = {};
    ...
    const requests = subjects.map(function (s) {
      return fetchJson(
    ...
    Promise.all(requests)
      ...
      .catch(function (err) {
        console.error(err);
        $("#books").html(
          '<div class="empty">Could not load popular books.</div>'
        );
        $("#loadMoreWrap").hide();
      });
  })();
...
function renderPopularPage() {
  ...
  let output = "";
  $.each(slice, function (index, work) {
    let cover = "";
    if (work.cover_id) {
      ...
      cover =
        'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="300"><rect width="100%" height="100%" fill="%230b1220"/><text x="50%" y="50%" fill="%239fb0c8" font-family="Arial, sans-serif" font-size="16" dominant-baseline="middle" text-anchor="middle">No cover</text></svg>';
    }

    const title = work.title || "Untitled";
    const key =
      work.key ||
      (work.cover_edition_key ? `/works/${work.cover_edition_key}` : "");

    let cardAuthors = "";
    if (Array.isArray(work.authors) && work.authors.length) {
      cardAuthors = work.authors
        .map(function (a) {
          return a && (a.name || (a.author && a.author.name))
      ...
      <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
        <div class="card text-center h-100 fade-in">
          ...
          <div class="card-body">
            ...
            ${
              cardAuthors
                ? `<div class="muted" style="font-size:0.8rem">${cardAuthors}</div>`
                : ""
            }
...
function getBooks(searchText) {
  fetchJson(
    ...
    .then(function (response) {
      const books = (
        (response && response.data && response.data.docs) ||
        []
      ).slice(0, 24);
      let output = "";
        ...
        output += `
                <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
                  <div class="card text-center h-100">
                    <img src="${cover}" class="card-img-top book-cover" alt="${
          book.title
        }">
              <div class="card-body">
                ...
                ${
                  authors
                    ? `<div class="muted" style="font-size:0.8rem">${authors}</div>`
                    : ""
                }
                  <a onclick="bookSelected('${
                    book.key
                  }','${encoded}')" href="#" class="btn btn-primary btn-sm">Details</a>
                  <button class="btn btn-outline-light btn-sm js-add-fav" data-key="${
                    book.key
                  }" data-authors="${encoded}" data-title="${encodeURIComponent(
          book.title || ""
        )}" data-cover="${encodeURIComponent(cover)}">☆ Favourites</button>
              </div>
            </div>
          </div>
        `;
...
function removeFavourite(key) {
  const list = _loadFavourites();
  const remaining = list.filter(function (i) {
...
function addToFavouritesFromDetail(
  key,
  encodedAuthors,
  encodedTitle,
  encodedCover
